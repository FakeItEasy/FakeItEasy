namespace FakeItEasy.Specs
{
    ...
    public class FakingInternals
    {
        ...
        public void when_trying_to_fake_invisible_internals(
            Exception exception)
        {
            "when trying to fake invisible internals".x(() =>
            {
                exception = Catch.Exception(() => A.Fake<IInternal>());
            });

            "it should throw an exception with a message containing a hint at using internals visible to attribute".x(() =>
            {
            ...
            Exception exception)
        {
            "when trying to fake invisible internals".x(() =>
            {
                exception = Catch.Exception(() => A.Fake<IList<IInternal>>());
            });
            ...
            Exception exception)
        {
            "establish".x(() =>
            {
                fake = A.Fake<TypeWithInternalMethod>();
            });

            "when trying to fake invisible internals".x(() =>
            {
                exception = Catch.Exception(() => A.CallTo(() => fake.InternalMethod()).Returns(17));
            });

            "it should throw an exception with a message complaining about accessibility".x(() =>
            {
