namespace FakeItEasy.Specs.ArgumentConstraints
{
    ...
    public static class IsEquivalentToSpecs
    {
        ...
        public static void Success_SameSequence(IIHaveACollectionParameter fake)
        {
            ...
            "Then an assertion that it was called with an array equivalent to the exact same sequence should pass"
                .x(() => A.CallTo(() => fake.Bar(A<int[]>.That.IsEquivalentTo(new[] { 1, 2, 3 }))).MustHaveHappened());
        }
        ...
        public static void Success_SameElementsDifferentOrder(IIHaveACollectionParameter fake)
        {
            ...
            "Then an assertion that it was called with an array equivalent to the same items in a different order should pass"
                .x(() => A.CallTo(() => fake.Bar(A<int[]>.That.IsEquivalentTo(new[] { 1, 2, 2, 3 }))).MustHaveHappened());
        }
        ...
        public static void Success_SameElementsDifferentOrder_UsingParamsOverload(IIHaveACollectionParameter fake)
        {
            ...
            "Then an assertion that it was called with an array equivalent to the same items in a different order should pass"
                .x(() => A.CallTo(() => fake.Bar(A<int[]>.That.IsEquivalentTo(1, 2, 2, 3))).MustHaveHappened());
        }
        ...
        public static void Success_SameElements_DifferentOrder_WithEqualityComparer(IIHaveACollectionParameter fake)
        {
            ...
            "Then an assertion that it was called with an array equivalent to the same items in a different order, using a specific equality comparer, should pass"
                .x(() => A.CallTo(() => fake.Bar(A<int[]>.That.IsEquivalentTo(
                    new[] { 1, 2, 2, 3 },
        ...
        public static void Failure_DifferentElements(IIHaveACollectionParameter fake, Exception? exception)
        {
            ...
            "And an assertion is made that it was called with an array equivalent to a different set of items"
                .x(() => exception = Record.Exception(() =>
                    A.CallTo(() => fake.Bar(A<int[]>.That.IsEquivalentTo(new[] { 4, 5, 6 }))).MustHaveHappened()));
            ...
            "And the exception message should tell us that the call was not matched, and include the values of the actual collection elements"
                .x(() => exception!.Message.Should().BeModuloLineEndings("""
                      ...
                      Assertion failed for the following call:
                        FakeItEasy.Specs.ArgumentConstraints.IsEquivalentToSpecs+IIHaveACollectionParameter.Bar(args: <4, 5, 6 (in any order)>)
                      Expected to find it once or more but didn't find it among the calls:
                        1: FakeItEasy.Specs.ArgumentConstraints.IsEquivalentToSpecs+IIHaveACollectionParameter.Bar(args: [1, 2, 3])
            ...
            "And the exception message should tell us that the call was not matched, and include the values of the actual collection elements"
                .x(() => exception!.Message.Should().BeModuloLineEndings("""
                      ...
                      Assertion failed for the following call:
                        FakeItEasy.Specs.ArgumentConstraints.IsEquivalentToSpecs+IIHaveACollectionParameter.Bar(args: <4, 5, 6 (in any order)>)
                      Expected to find it once or more but didn't find it among the calls:
                        1: FakeItEasy.Specs.ArgumentConstraints.IsEquivalentToSpecs+IIHaveACollectionParameter.Bar(args: NULL)
